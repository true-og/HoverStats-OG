...
public class HoverUtils {
    ...
    public static TextComponent setupHoverChatMessage(Player p, String message) {
        ...
        String suggestCmd = plugin.getConfig().getString("Hover.Stats Click Command", "/msg %player% ")
                .replace("%displayname%", p.displayName().examinableName()).replace("%player%", p.getName());
    ...
    public static TextComponent setupHoverJoinMessage(Player p, String message) {
        ...
        String suggestCmd = plugin.getConfig().getString("Hover.Join Click Command", "Welcome back")
                .replace("%displayname%", p.displayName().examinableName()).replace("%player%", p.getName());
    ...
    public static TextComponent setupHoverFirstJoinMessage(Player p, String message) {
        ...
        String suggestCmd = plugin.getConfig().getString("Hover.First Join Click Command", "Welcome to the Server!")
                .replace("%displayname%", p.displayName().examinableName()).replace("%player%", p.getName());
    ...
    public static TextComponent setupHoverLeaveMessage(Player p, String message) {
        ...
        String suggestCmd = plugin.getConfig().getString("Hover.Quit Click Command", "/mail %player% ")
                .replace("%displayname%", p.displayName().examinableName()).replace("%player%", p.getName());
    ...
    private static HoverEvent formatHoverMessage(Player p, List<String> strList) {
        ...
        for (int i = 0; i < strList.size(); i++) {
            ...
            // Add in any placeholders that can be added to the message as well as
            ...
            if (i < strList.size() - 1) {
                ...
                // Since this isn't the final output add a line break to make each line give the
