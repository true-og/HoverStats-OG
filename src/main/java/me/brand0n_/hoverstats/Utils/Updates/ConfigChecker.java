...
public class ConfigChecker {
    ...
    public static void checkConfig() {
        ...
        if (!plugin.getConfig().getString("Chat Formatting.Format", "&7%displayname% &8&l> &7%message%")
                .contains("%message%"))
        {
    ...
    public static void checkUpdates() {
        ...
        File newConfig = new File(plugin.getDataFolder() + "/old configs",
                fileName.replace("%num%", String.valueOf(num)));
    ...
    public static boolean isOutOfDate(String earlierVersion) {
        ...
        // Check if the current version of the config is equal to the defined config
        // version
        if (!earlierVersion.equalsIgnoreCase(plugin.getPluginMeta().getVersion().replace(" BETA", "").trim())) {
    ...
    /**
     * This method will format the date to the following format: <br />
     * (year-day-month) <br />
     * <br />
     *
    ...
    /**
     * This method creates a formatted version of the old file, using the current
     * date and checking if there are any other files that already exist. <br />
     * <br />
     * If the file already exists then this method will add a number after the file
     * name to differentiate the files. <br />
     * <br />
     *
    ...
    private static String fileVersion(File newFile, String fileName, int num) {
        ...
        if (newFile.exists()) {
            ...
            do {
                ...
                newFile.renameTo(new File(plugin.getDataFolder() + "/old configs",
                        fileName.replace("%num%", String.valueOf(num))));
    ...
    /**
     * This method creates the new file and saves it to the file <br />
     * "/old configs/[date](version).yml". <br />
     * <br />
     *
     * @return the file that was created <br />
     *         if no file was created then the return will be "null"
     */
    private static File createFile(File newFile) {
        ...
        try {
            ...
            if (!newFile.getParentFile().exists()) {
                ...
                Bukkit.getServer().getConsoleSender().sendMessage(Colors.chatColor(
                        "[" + plugin.getName() + "]" + "&9⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯[ &fParent File Issue &9]⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯"));
                // Tell user the parent file is being created
                Bukkit.getServer().getConsoleSender().sendMessage(Colors.chatColor("[" + plugin.getName() + "]"
                ...
                // Print barrier
                Bukkit.getServer().getConsoleSender().sendMessage(Colors
                        .chatColor("[" + plugin.getName() + "]" + "&9⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯"));
            ...
            Bukkit.getServer().getConsoleSender().sendMessage(
                    Colors.chatColor("[" + plugin.getName() + "]" + " &eINFO  &8| &7Destination File created."));

        } catch (IOException e) {

            // Print barrier
            Bukkit.getServer().getConsoleSender().sendMessage(
                    Colors.chatColor("[" + plugin.getName() + "]" + "&9⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯"));
    ...
    /**
     * This method copies data from one file to another file <br />
     * <br />
     *
    ...
    /**
     * This method deletes the old file, if the file cannot be deleted it will
     * remove the newly created config as well <br />
     * "/old configs/[date](version).yml". <br />
     * <br />
     *
     * @param newFile the new file that was created <br />
     *                <br />
     * @return true if the file was successfully deleted & <br />
     *         false if the file wasn't able to be deleted
     */
    private static boolean deleteOldConfig(File newFile) {
        ...
        if (!oldConfig.delete()) {
            ...
            if (newFile.delete()) {
                ...
                Bukkit.getServer().getConsoleSender().sendMessage(Colors
                        .chatColor("[" + plugin.getName() + "]" + " &eINFO  &8| &7New configuration file cleaned up."));
                ...
                Bukkit.getServer().getConsoleSender().sendMessage(Colors.chatColor(
                        "[" + plugin.getName() + "]" + " &cERROR &8| &7Unable to delete the new configuration file."));
    ...
    /**
     * This method saves all data from previous versions of the plugin into the
     * config file <br />
     * <br />
     *
    ...
    /**
     * This method loops through all the data in each key set saving the solo keys
     * as it goes <br />
     * <br />
     *
        ...
        for (String data : configSec.getKeys(false)) {
            ...
            if (!(configSec.get(data) instanceof ConfigurationSection)) {
                ...
                if (plugin.getConfig().contains(element)) {
                    ...
                    // Element isn't a config section, set the config file to be equal to the old
                    // data
